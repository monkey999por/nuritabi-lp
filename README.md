...
「行った場所で、日本を塗れ。」— 地図上でスポット（ピン）をつないで旅行単位・日単位の旅程と記録をつくり、その先に日本全国の制覇マップと訪問回数が積み上がるサービス『塗り旅』のウェイティングリスト用ランディングページ。スポットは有名観光地から名前のない展望地・お気に入りの路肩まで、地図にピンを打てる場所ならどこでも記録できます。
